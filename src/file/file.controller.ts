...
export class FileController {
  ...
  async loadGoogleFile(
    ...
    if (!path.startsWith(CloudProvidersMetaData.GOOGLE)) {
      throw new BadRequestException(`Invalid path for Google Cloud Provider: ${path}`);
    }
  ...
  async loadAwsFile(
    ...
    if (!path.startsWith(CloudProvidersMetaData.AWS)) {
      throw new BadRequestException(`Invalid path for AWS Cloud Provider: ${path}`);
    }
    const file: Stream = await this.loadCPFile(
  ...
  async loadAzureFile(
    ...
    if (!path.startsWith(CloudProvidersMetaData.AZURE)) {
      throw new BadRequestException(`Invalid path for Azure Cloud Provider: ${path}`);
    }
    const file: Stream = await this.loadCPFile(
  ...
  async loadDigitalOceanFile(
    ...
    if (!path.startsWith(CloudProvidersMetaData.DIGITAL_OCEAN)) {
      throw new BadRequestException(`Invalid path for Digital Ocean Cloud Provider: ${path}`);
    }
    const file: Stream = await this.loadCPFile(
