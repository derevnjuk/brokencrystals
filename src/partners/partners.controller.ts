...
export class PartnersController {
  ...
  async partnerLogin(
    ...
    try {
      const xpath = `//partners/partner[username/text()=${this.toXPathLiteral(username)} and password/text()=${this.toXPathLiteral(password)}]/*`;
  ...
  async searchPartners(@Query('keyword') keyword: string): Promise<string> {
    ...
    try {
      const xpath = `//partners/partner/name[contains(., ${this.toXPathLiteral(keyword)})]`;
      return this.partnersService.getPartnersProperties(xpath);
    } catch (err) {
      const errStr = err.toString();
      const errorMessage =
