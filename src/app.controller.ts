...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      // Implement allowlist validation to prevent SSTI
      const allowedTemplates = ['template1', 'template2']; // Example allowlist
      if (!allowedTemplates.includes(text)) {
        throw new HttpException('Invalid template', HttpStatus.BAD_REQUEST);
      }
