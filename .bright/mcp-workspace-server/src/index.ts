...
async function brightRequest(
  ...
  const res = await fetch(`https://${BRIGHT_HOSTNAME}${path}`, {
    method,
    headers: {
      // Bright's application API (scans, repeaters) authenticates with the Api-Key scheme,
      // the same way the Bright CLI uses the token. (The OAuth revoke/token endpoints use
      // Bearer, but those are handled in the workflow YAML, not here.)
