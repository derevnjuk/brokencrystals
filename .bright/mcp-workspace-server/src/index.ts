...
server.registerTool('commit_changes', {
  description: 'Commit and push the fix to the branch. Pass the exact repo-relative paths you changed; never commits build artifacts (node_modules, logs).',
  inputSchema: {
    message: z.string().min(1).describe('Commit message describing the change.'),
    paths: z.array(z.string()).optional().describe('Repo-relative paths to commit (the files you edited/added/deleted).')
  }
}, async ({ message, paths }) => {
  const start = Date.now();
  ...
  try {
    ...
    const staged = await execFileAsync('git', ['diff', '--cached', '--name-only'], opts);
    if (staged.stdout.trim() === '') {
      log('tool.ok', { tool: 'commit_changes', committed: false, ms: Date.now() - start });

      return { content: [{ type: 'text', text: JSON.stringify({ committed: false, reason: 'no staged changes' }) }] };
    }
